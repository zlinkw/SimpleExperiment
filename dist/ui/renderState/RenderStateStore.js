...
exports.firstNonEmptyRecord = firstNonEmptyRecord;
exports.firstNonEmptyArray = firstNonEmptyArray;
exports.normalizeArray = normalizeArray;
exports.numberOrUndefined = numberOrUndefined;
exports.percent = percent;
exports.formatDuration = formatDuration;
exports.transferRateBytesPerSecond = transferRateBytesPerSecond;
...
function percent(used, total) {
    const u = numberOrUndefined(used);
    const t = numberOrUndefined(total);
    if (u === undefined || !t)
        return undefined;
...
function formatDuration(startedAt, updatedAt) {
    ...
    const e = Date.parse(String(updatedAt || "")) || Date.now();
...
function transferRateBytesPerSecond(t, s, u) {
    const b = numberOrUndefined(t);
    const start = Date.parse(String(s || ""));
    if (b === undefined || b <= 0 || !Number.isFinite(start))
        return undefined;
    const end = Date.parse(String(u || "")) || Date.now();
    const sec = (end - start) / 1000;
    return sec > 0 ? Math.round(b / sec) : undefined;
}
function transferEtaSeconds(done, total, rate) {
    const d = numberOrUndefined(done);
    const t = numberOrUndefined(total);
    const r = numberOrUndefined(rate);
    if (d === undefined || t === undefined || r === undefined || r <= 0 || t <= d)
        return undefined;
...
function operationStatusFromType(type) {
    ...
    if (v.includes("failed"))
        return "failed";
    if (v.includes("cancelled") || v.includes("canceled"))
        return "cancelled";
    if (v.includes("stalled"))
        return "stalled";
    if (v.includes("started") || v.includes("progress"))
        return "running";
    return "-";
}
...
function normalizeFileTransferRows(fileTransfers) {
    ...
    return rows.map((row) => {
        ...
        const sAt = pick(row, ["startedAt", "started_at"], "");
        const uAt = pick(row, ["updatedAt", "updated_at", "finishedAt", "finished_at"], "");

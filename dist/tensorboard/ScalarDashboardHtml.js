...
.group{margin:0 0 15px}.group>summary{font-weight:700;font-size:15px;padding:7px 2px;cursor:pointer}.chart-grid{display:grid;grid-template-columns:repeat(var(--chart-columns,3),minmax(0,1fr));gap:12px}.chart-card{min-width:0;background:#fff;border:1px solid #dce3ed;border-radius:8px;padding:0 11px 11px}.chart-card.full-row{grid-column:1/-1}.chart-card>summary{cursor:pointer;font-weight:700;padding:11px 2px;overflow-wrap:anywhere}.chart-card .toolbar{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:8px;font-size:12px}.chart-card .toolbar label{display:inline-flex;align-items:center;gap:3px}.chart-card .toolbar input[type=range]{width:75px}.chart-card .toolbar .layout-button{margin-left:auto;font-size:12px;padding:3px 8px}.chart-card canvas{display:block;width:100%;height:295px}.legend{display:flex;flex-wrap:wrap;gap:5px;min-height:22px}.pill{font-size:11px;border-radius:12px;background:#edf1f7;padding:2px 6px}.tooltip{container-type:inline-size;min-width:0;min-height:36px;max-height:min(48vh,520px);overflow-y:auto;overscroll-behavior:contain;color:#334155}.hover-hint{font-size:11px;padding:5px 0;color:#64748b}.hover-entry{border:1px solid #e1e7f0;border-left:4px solid var(--series-color);border-radius:6px;background:#f9fbfe;padding:7px 9px;margin-top:6px;min-width:0}.hover-heading{display:flex;flex-wrap:wrap;align-items:baseline;gap:3px 10px;margin-bottom:5px;min-width:0}.hover-case{font-weight:650;font-size:clamp(12px,calc(9px + 1cqw),15px);overflow-wrap:anywhere;min-width:0}.hover-step{font-size:clamp(11px,calc(8px + 1cqw),13px);color:#64748b;white-space:nowrap}.hover-metrics,.hover-seeds{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,105px),1fr));gap:4px 8px;margin:0}.hover-metric,.hover-seed{min-width:0;display:flex;justify-content:space-between;gap:5px;font-size:clamp(11px,calc(8px + 1cqw),13px);font-variant-numeric:tabular-nums}.hover-metric dt,.hover-seed dt{color:#64748b;white-space:nowrap}.hover-metric dd,.hover-seed dd{margin:0;text-align:right;overflow-wrap:anywhere}.hover-seeds{border-top:1px solid #e5eaf2;margin-top:5px;padding-top:5px}@container (max-width:390px){.hover-heading{display:block}.hover-step{display:block;margin-top:2px}.hover-metrics,.hover-seeds{grid-template-columns:repeat(auto-fit,minmax(min(100%,88px),1fr))}}.native{display:block;width:100%;height:calc(100vh - 59px);border:0;background:#fff}
...
(function(){
  ...
  function hoverMetric(parent,label,value,seed=false){
    const item=el('div',seed?'hover-seed':'hover-metric');item.appendChild(el('dt','',label));item.appendChild(el('dd','',String(value)));parent.appendChild(item);
  }
  function renderHoverDetails(card,entries){
    if(!entries.length){showHoverHint(card);return}
    const fragment=document.createDocumentFragment();
    for(const entry of entries){
      const row=el('section','hover-entry');row.style.setProperty('--series-color',entry.color);
      const heading=el('div','hover-heading');heading.appendChild(el('span','hover-case',entry.case));heading.appendChild(el('span','hover-step',(entry.label?entry.label+' · ':'')+'step '+entry.step));row.appendChild(heading);
      const metrics=el('dl','hover-metrics');
      if(entry.value!==undefined)hoverMetric(metrics,'值',Number(entry.value).toPrecision(6));
      if(entry.mean!==undefined)hoverMetric(metrics,'均值',Number(entry.mean).toPrecision(6));
      if(entry.smooth!==undefined)hoverMetric(metrics,'平滑',Number(entry.smooth).toPrecision(6));
      if(entry.std!==undefined)hoverMetric(metrics,'标准差',entry.std===null?'—':Number(entry.std).toPrecision(5));
      if(entry.n!==undefined)hoverMetric(metrics,'参与 seed',entry.n+'/'+(entry.expectedSeeds||'?'));
      row.appendChild(metrics);
      const seeds=Object.entries(entry.seeds||{});
      if(seeds.length){const list=el('dl','hover-seeds');seeds.forEach(([seed,value])=>hoverMetric(list,'seed '+seed,Number(value).toPrecision(4),true));row.appendChild(list)}
      fragment.appendChild(row);
    }
    card.tooltip.replaceChildren(fragment);
  }
  function hoverCard(card,event){
    ...
    for(const line of card.displaySeries||[]){
      ...
      if(points.length){
        ...
        entries.push({case:row.case,color:line.color,step:point.step,mean:point.mean,smooth:Number(card.smooth.value)>0?line.means[index]:undefined,std:point.std,n:point.n,expectedSeeds:row.expectedSeeds,seeds:point.seeds});
      }else if(row.rawOnly)(row.seeds||[]).forEach(seed=>{
        const values=seed.points||[];
        if(values.length){const point=values[nearestIndex(values,step,value=>value[0])];entries.push({case:row.case,color:line.color,label:'seed '+seed.seed,step:point[0],value:point[1]})}
      });
    }

...
(function(){
  ...
  function renderHoverDetails(card,entries){
    ...
    const seedPairs=entries.map(entry=>Object.entries(entry.seeds||{}).map(([seed,value])=>[seed,Number(value).toPrecision(4)]));
    const seedValues=seedPairs.map(pairs=>pairs.map(([seed,value])=>seed+':'+value).join(' · '));
    const seedChars=Math.max(12,...seedValues.map(value=>textWidth(value)+2));
    const seedLimit=Math.max(12,Math.floor(available*.48/7));
    table.style.setProperty('--hover-seeds-width',Math.min(90,seedLimit,seedChars)+'ch');
    const columns=el('colgroup');['case-column','step-column','','extra-column','extra-column','','seed-column'].forEach(cls=>columns.appendChild(el('col',cls)));table.appendChild(columns);
    [['case',''],['step','step-column'],['数值',''],['平滑','extra-column'],['标准差','extra-column'],['seed',''],['各 seed 值','seed-column']].forEach(([label,cls])=>heading.appendChild(el('th',cls,label)));
    head.appendChild(heading);table.appendChild(head);
    const body=el('tbody');
    for(const [index,entry] of entries.entries()){
      const row=el('tr');row.style.setProperty('--series-color',entry.color);
      const seeds=seedValues[index];
      const values=[entry.case+(entry.label?' · '+entry.label:''),String(entry.step),Number(entry.value??entry.mean).toPrecision(6),entry.smooth===undefined?'—':Number(entry.smooth).toPrecision(6),entry.std===undefined||entry.std===null?'—':Number(entry.std).toPrecision(5),entry.n===undefined?'—':entry.n+'/'+(entry.expectedSeeds||'?'),seeds||'—'];
      values.forEach((value,columnIndex)=>{
        const cls=columnIndex===1?'step-column':columnIndex===3||columnIndex===4?'extra-column':columnIndex===6?'seed-column':'';
        const cell=el('td',cls);cell.title=value;
        if(columnIndex===6&&seedPairs[index].length){
          const list=el('span','hover-seed-list');
          seedPairs[index].forEach(([seed,seedValue])=>{const pair=el('span','hover-seed-pair');pair.appendChild(el('strong','hover-seed-key',seed+':'));pair.appendChild(el('span','hover-seed-value',seedValue));list.appendChild(pair)});
          cell.appendChild(list);
        }else cell.textContent=value;
        row.appendChild(cell);
      });
      row.title='case '+values[0]+' · step '+values[1]+' · 数值 '+values[2]+' · 平滑 '+values[3]+' · 标准差 '+values[4]+' · seed '+values[5]+' · '+values[6];

...
(function(){
  ...
  function renderHoverDetails(card,entries){
    ...
    for(const entry of entries){
      ...
      const seeds=Object.entries(entry.seeds||{}).map(([seed,value])=>seed+':'+Number(value).toPrecision(4)).join(' · ');
      ...
      values.forEach((value,index)=>{const cls=index===1?'step-column':index===3||index===4?'extra-column':index===6?'seed-column':'';const cell=el('td',cls,value);cell.title=value;row.appendChild(cell)});

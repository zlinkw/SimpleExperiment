...
exports.defaultXshellTunnelSetupConfig = void 0;
exports.normalizeXshellSetupConfig = normalizeXshellSetupConfig;
exports.normalizeXshellWorkerTunnelConfig = normalizeXshellWorkerTunnelConfig;
exports.workerTunnelToXshellSetupConfig = workerTunnelToXshellSetupConfig;
exports.validateXshellSetupConfig = validateXshellSetupConfig;
exports.xshellExecutablePath = xshellExecutablePath;
exports.publicXshellSetupSummary = publicXshellSetupSummary;
const LegacyTunnelCompat_1 = require("./LegacyTunnelCompat");
...
exports.defaultXshellTunnelSetupConfig = {
    ...
    workerTunnels: [],
    ports: {
        workerLocalPortRange: TunnelPortConflict_1.defaultTunnelPorts.workerLocalPortRange,
        assignments: [],
        preserveExistingAssignments: true,
    },
    realtime: WorkerTelemetryApi_1.defaultMultiWorkerRealtimePolicy,
};
function normalizeXshellSetupConfig(input = {}) {
    ...
    const workerTelemetryMode = input.workerTelemetryMode === "hub_plus_worker_telemetry" || input.workerRealtimeMode === "hub_plus_workers"
        ...
        : "hub_only";
    const xshellExePath = (input.xshellExePath || (0, LegacyTunnelCompat_1.legacyTunnelString)(input, "ExePath") || "").trim();
    const xshellSessionName = (input.xshellSessionName || (0, LegacyTunnelCompat_1.legacyTunnelString)(input, "SessionName") || "").trim() || undefined;
    const cleaned = (0, LegacyTunnelCompat_1.omitLegacyTunnelKeys)({ ...input }, ["ExePath", "SessionName"]);
    return {
        ...exports.defaultXshellTunnelSetupConfig,
        ...cleaned,
        xshellExePath,
        ...
        hubSshPort: normalizeSshPort(input.hubSshPort, exports.defaultXshellTunnelSetupConfig.hubSshPort),
        localForwardHost: "127.0.0.1",
        localForwardPort: (0, TunnelGateway_1.normalizePort)(input.localForwardPort, exports.defaultXshellTunnelSetupConfig.localForwardPort),
        remoteAgentHost: "127.0.0.1",
        remoteAgentPort: (0, TunnelGateway_1.normalizePort)(input.remoteAgentPort, exports.defaultXshellTunnelSetupConfig.remoteAgentPort),
        xshellSessionName,
...
function normalizeXshellWorkerTunnelConfig(input = {}, index = 0, fallbackRemoteAgentPort = exports.defaultXshellTunnelSetupConfig.remoteAgentPort) {
    ...
    const workerSshPort = normalizeSshPort(input.workerSshPort || input.hubSshPort, exports.defaultXshellTunnelSetupConfig.hubSshPort);
    const remoteTelemetryPort = (0, TunnelGateway_1.normalizePort)(input.remoteTelemetryPort || input.remoteAgentPort, (0, TunnelGateway_1.normalizePort)(fallbackRemoteAgentPort, TunnelPortConflict_1.defaultTunnelPorts.defaultWorkerTelemetryPort));
    const xshellSessionName = (input.xshellSessionName || (0, LegacyTunnelCompat_1.legacyTunnelString)(input, "SessionName") || "").trim() || undefined;
    const cleaned = (0, LegacyTunnelCompat_1.omitLegacyTunnelKeys)({ ...input }, ["SessionName"]);
    return {
        ...cleaned,
        id,
        ...
        sshHost: input.sshHost?.trim() || undefined,
        localForwardHost: "127.0.0.1",
        localForwardPort: (0, TunnelGateway_1.normalizePort)(input.localForwardPort, TunnelPortConflict_1.defaultTunnelPorts.workerLocalPortRange.start + index),
        remoteAgentHost: "127.0.0.1",
...
function workerTunnelToXshellSetupConfig(base, worker) {
...
function validateXshellSetupConfig(config) {
    const errors = [];
    if (!xshellExecutablePath(config).trim())
        errors.push("请选择 Xshell.exe。");
    if (config.launchMode === "open_saved_session" && !config.savedSessionPath)
        errors.push("Xshell 已保存会话模式需要选择 Hub 的 .xsh 会话文件。");
    if (config.launchMode !== "open_saved_session" && !config.sshConfigAlias && !config.hubHost.trim())
        errors.push("请输入服务器 IP 或域名，或选择 Xshell 会话文件。");
    if (config.launchMode !== "open_saved_session" && !config.sshConfigAlias && !config.hubUser.trim())
        errors.push("请输入登录用户名。");
    if (config.hubSshPort < 1 || config.hubSshPort > 65535)
        errors.push("服务器 SSH 登录端口必须在 1-65535 之间。");
    if (config.localForwardPort < 1024 || config.localForwardPort > 65535)
        errors.push("本地转发端口必须在 1024-65535 之间。");
    if (config.remoteAgentPort < 1024 || config.remoteAgentPort > 65535)
        errors.push("服务器上的 Agent 端口必须在 1024-65535 之间。");
    if (config.localForwardHost !== "127.0.0.1" || config.remoteAgentHost !== "127.0.0.1")
        errors.push("隧道两端只能使用 127.0.0.1。");
    if (config.authMethod === "key" && !config.privateKeyPath)
        errors.push("密钥登录需要选择私钥文件。");
    if (config.workerRealtimeMode === "hub_plus_workers" && !config.workerTunnels.some((worker) => worker.enabled))
        errors.push("Worker 实时模式至少需要一个已启用的 Worker 隧道。");
    for (const worker of config.workerTunnels.filter((item) => item.enabled)) {
        if (config.launchMode === "open_saved_session" && !worker.savedSessionPath)
            errors.push(`Worker ${worker.id} 需要选择 Xshell 会话文件。`);
        if (config.launchMode !== "open_saved_session" && !worker.sshConfigAlias && !(worker.workerHost || worker.hubHost).trim())
            errors.push(`Worker ${worker.id} 需要填写服务器地址或登录别名。`);
        if (config.launchMode !== "open_saved_session" && !worker.sshConfigAlias && !(worker.workerUser || worker.hubUser).trim())
            errors.push(`Worker ${worker.id} 需要填写登录用户名。`);
        if (worker.authMethod === "key" && !worker.privateKeyPath)
            errors.push(`Worker ${worker.id} 使用密钥登录时需要选择私钥文件。`);
    }
    return errors;
}
function xshellExecutablePath(config) {
    return (config.xshellExePath || (0, LegacyTunnelCompat_1.legacyTunnelString)(config, "ExePath") || "").trim();
}
function publicXshellSetupSummary(config) {
    const root = (0, LegacyTunnelCompat_1.omitLegacyTunnelKeys)(config, ["ExePath", "SessionName"]);
    return {
        ...root,
        ...
        workerTunnels: config.workerTunnels.map((worker) => ({
            ...(0, LegacyTunnelCompat_1.omitLegacyTunnelKeys)(worker, ["SessionName"]),
...
function normalizeSavedSessionRunner(_value) {
    return "xshell";
}
...
function normalizeForwardIndex(value) {
...
function normalizePortConfig(input, workers, hubLocalPort, hubRemotePort, hubDisplayName) {
    ...
    for (const worker of workers) {
        if (assignments.some((assignment) => assignment.endpointId === worker.id))
            continue;
        assignments.push({
...
function dedupeWorkerTunnels(workers) {
    const out = [];
    for (const worker of workers) {
        ...
        if (index < 0)
            ...
            out[index] = mergeWorker(out[index], worker);
    }
    return out;
}
function isDuplicateWorker(first, second) {
    if (first.savedSessionPath && second.savedSessionPath && pathKey(first.savedSessionPath) === pathKey(second.savedSessionPath))
        return true;
    const firstHost = hostKey(first);
    const secondHost = hostKey(second);
    return Boolean(firstHost && secondHost && firstHost === secondHost);
}
function mergeWorker(first, second) {

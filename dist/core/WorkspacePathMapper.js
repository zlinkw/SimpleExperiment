...
exports.resolveWorkspaceLocation = resolveWorkspaceLocation;
...
function resolveWorkspaceLocation(uri, config = {}) {
    ...
    const relativePath = node_path_1.default.posix.relative(containerRoot, containerPath);
    if (isEscapingRelativePath(relativePath, node_path_1.default.posix)) {
    ...
    const hostPath = node_path_1.default.win32.resolve(hostRoot, ...segments);
    const hostRelative = node_path_1.default.win32.relative(hostRoot, hostPath);
    if (isEscapingRelativePath(hostRelative, node_path_1.default.win32)) {
...
function normalizeWindowsAbsolutePath(value, label) {
    ...
    return node_path_1.default.win32.normalize(raw);
}
...
function normalizeContainerPath(value, label) {
    ...
    const normalized = node_path_1.default.posix.normalize(decoded);

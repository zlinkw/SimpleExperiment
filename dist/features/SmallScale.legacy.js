...
function buildCompletenessMatrix(config, input) {
    ...
    const gateResultsByExperiment = new Map();
    for (const [order, value] of (input.gateResults || []).entries()) {
        const gates = gateResultsByExperiment.get(value.experimentId);
        if (gates)
            gates.push({ order, value });
        else
            gateResultsByExperiment.set(value.experimentId, [{ order, value }]);
    }
    const lifecyclesByExperiment = new Map();
    for (const [order, value] of (input.lifecycles || []).entries()) {
        const lifecycles = lifecyclesByExperiment.get(value.experimentId);
        if (lifecycles)
            lifecycles.push({ order, value });
        else
            lifecyclesByExperiment.set(value.experimentId, [{ order, value }]);
    }
...
function generateMissingOnlyRerunPlan(cells, options) {
    ...
    const record = (0, PlanBuilder_1.importLegacyPlanYamlToRegistry)(`simple_cluster/plans/generated/${suite}.yaml`, yaml);
...
function buildOutputCapabilityMatrix(report) {
    ...
    for (const file of report.files) {
        if (file.status !== "found")
            missingFileIds.add(file.specId);
    }
    const missingColumnNames = new Set();
    for (const column of report.columns) {
        if (column.status === "missing")
            missingColumnNames.add(column.column);
    }

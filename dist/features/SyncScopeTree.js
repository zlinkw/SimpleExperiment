...
function versionTime(version){return version?.modifiedAtMs?new Date(version.modifiedAtMs).toLocaleString():'时间未知'}
...
function paintStatus(badge,health,directory){if(!health){badge.textContent='待刷新';return}if(health.state==='same')return;if(directory){const parts=health.detail.split(' · '),facts=[];for(const part of parts){const count=Number(part.match(/^([0-9]+) /)?.[1]||0);if(!count)continue;if(part.includes('待更新或冲突'))facts.push(count+' 个文件内容不同或缺失');else if(part.includes('仅 Worker 一致'))facts.push(count+' 个文件本机缺失或不同');else if(part.includes('未确认'))facts.push(count+' 个文件未完成校验');else if(part.includes('范围外'))facts.push(count+' 个文件未纳入同步范围')}const outside=health.detail.match(/^当前同步范围外 · ([0-9]+) 个文件$/);for(const [id,copy] of Object.entries(health.copies||{})){const issues=[copy.missing?'缺失 '+copy.missing:'',copy.needsSync?'需同步 '+copy.needsSync:'',copy.conflict?'内容冲突 '+copy.conflict:'',copy.unverified?'未校验 '+copy.unverified:''].filter(Boolean);facts.push((id==='local'?'本机':id)+' 最新文件 '+versionTime(copy)+(issues.length?' · '+issues.join(' · '):''))}badge.textContent=facts.join(' · ')||(outside?outside[1]+' 个文件未纳入同步范围':health.detail);return}for(const part of health.detail.split(' · ')){if(part==='Worker 内容基准'||part.startsWith('Plan 归属：'))continue;let text=part;const copy=part.match(/^(本机|[^ ]+) (最新版|同版|缺失|不同版|待更新|冲突|未校验(?:，待核对)?)$/);if(copy){const id=copy[1],version=health.versions?.[id==='本机'?'local':id],role=versionRole(version);const issue=copy[2]==='缺失'||!version&&copy[2]==='待更新'?'缺失':copy[2]==='待更新'||copy[2]==='不同版'?'需同步':copy[2]==='冲突'?'内容冲突':copy[2].startsWith('未校验')?'未校验':'';text=id+(version?' '+versionTime(version):'')+(role?' · '+role:'')+(issue?' · '+issue:'')}else if(part==='内容冲突，无法判定最新版')text='最新版未确定';const item=document.createElement('span');item.textContent=text;badge.appendChild(item)}}

...
test("public setup defaults to system Python without changing explicit Conda environments", () => {
  ...
  assert.equal(packageJson.contributes.configuration.properties["zlkCluster.tunnel.condaEnv"].default, "");
...
blank = agent.zlk_runtime_env({})
explicit = agent.zlk_runtime_env({"ZLK_CONDA_ENV": "torch2"})
print(json.dumps({
    "blank": blank,
    "blank_python": agent.zlk_runtime_python(blank),
    "explicit": explicit,
    "explicit_python": agent.zlk_runtime_python(explicit),
    "agent_activation": agent.zlk_conda_activation_script(),
    "scheduler_blank_name": scheduler.zlk_conda_env_name({}),
    "scheduler_blank_activation": scheduler.zlk_conda_activation_script({}),
    "scheduler_blank_python": scheduler.runtime_python_command({}),
    "scheduler_explicit_activation": scheduler.zlk_conda_activation_script({"ZLK_CONDA_ENV": "torch2", "ZLK_REQUIRE_CONDA_ENV": "1"}),
}))
`;
  const result = spawnSync("python", ["-c", script], { encoding: "utf8" });
  assert.equal(result.status, 0, result.stderr || result.stdout);
  const value = JSON.parse(result.stdout.trim());
  assert.equal(value.blank.ZLK_CONDA_ENV, "");
  assert.equal(value.blank.ZLK_REQUIRE_CONDA_ENV, "0");
  assert.equal(value.blank_python, value.scheduler_blank_python);
  assert.equal(value.explicit.ZLK_CONDA_ENV, "torch2");
  assert.equal(value.explicit.ZLK_REQUIRE_CONDA_ENV, "1");
  assert.equal(value.explicit_python, "python");
  assert.match(value.agent_activation, /if \[ -n "\$ZLK_CONDA_ENV" \]; then :/);
  assert.equal(value.scheduler_blank_name, "");
  assert.equal(value.scheduler_blank_activation, "true");
  assert.match(value.scheduler_explicit_activation, /Conda env \$ZLK_CONDA_ENV is required/);
});
...
test("settings and confirmations explain the effective runtime environment", () => {
  ...
  assert.match(agentSource, /env\["ZLK_REQUIRE_CONDA_ENV"\] = "1" if conda_env else "0"/);

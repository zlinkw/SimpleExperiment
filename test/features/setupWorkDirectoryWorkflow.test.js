...
test("quick setup resolves Xshell and Worker blockers before Agent preparation", () => {
  ...
  assert.match(flow, /setupNext === "选择 Xshell 会话"[\s\S]{0,120}this\.configureXshellSavedSessions\(\)/);
  assert.match(flow, /workerNext === "添加 Worker"[\s\S]{0,120}this\.addWorkerConfigFromUi\(false\)/);
  ...
  assert.match(source, /仅填写 Hub 参数.*正式接入仍需选择带端口转发的 \.xsh 会话/);

...
test("scope Webview requests visible-file status and replaces waiting badges", () => {
  ...
  onMessage({ data: { type: "status", rootId: "workers", path: ".", statuses: { "data": { state: "remote-only", detail: "同步范围内 2 个文件 · 1 一致 · 0 待更新或冲突 · 1 仅 Worker 一致 · 0 未确认" }, "README.md": { state: "different", detail: "Plan 归属：w1 · 本机 缺失 · w1 最新版 · w2 待更新" } }, refreshedAt: "now" } });
  assert.equal(elements.get("tree").children[1].children.find((child) => child.textContent === "📁 data")?.className, "name remote-only");
  assert.equal(elements.get("tree").children[1].children.find((child) => child.className === "badge remote-only")?.textContent, "1 个文件本机缺失或不同");
  assert.equal(elements.get("tree").children[2].children.find((child) => child.textContent === "📄 README.md")?.className, "name different");
  assert.equal(elements.get("tree").children[2].children.find((child) => child.className === "badge different")?.children.map((child) => child.textContent).join(" · "), "Plan 归属：w1 · 本机 缺失 · w2 缺失");

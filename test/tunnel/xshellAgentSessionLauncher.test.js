...
test("xshell agent session preview opens saved xsh file only", () => {
  const preview = buildXshellSavedSessionPreview({
    ...
    sessionPath: "D:\\sessions\\zlk-hub-agent.xsh",
    displayName: "Hub Agent",
  });
  assert.match(preview, /Xshell\.exe/);
  assert.match(preview, /zlk-hub-agent\.xsh/);
...
test("agent session paths survive setup normalization", () => {
  const config = normalizeXshellSetupConfig({
    agentSessionPath: "D:\\sessions\\zlk-hub-agent.xsh",
    workerTunnels: [{
      id: "nwpu5",
      hubHost: "nwpu5",
      hubUser: "zlk",
      ...
      agentSessionPath: "D:\\sessions\\zlk-nwpu5-agent.xsh",
      enabled: true,
    }],
  });
  assert.equal(config.agentSessionPath, "D:\\sessions\\zlk-hub-agent.xsh");
  assert.equal(config.workerTunnels[0].agentSessionPath, "D:\\sessions\\zlk-nwpu5-agent.xsh");
});
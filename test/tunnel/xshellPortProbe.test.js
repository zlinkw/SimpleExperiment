...
test("worker telemetry port probe accepts read-only worker api", async () => {
  const server = http.createServer((req, res) => {
    res.setHeader("Content-Type", "application/json");
    if (req.url === "/api/health") return res.end(JSON.stringify({ schemaVersion: 1, agentVersion: "0.2.0", apiVersion: "1", mode: "worker_telemetry", startedAt: "x", serverTime: "x", uptimeSeconds: 1, projectRoot: "p", status: "ok" }));
...
test("worker telemetry port probe explains stale hub-mode agent", async () => {
  ...
  try {
    ...
    assert.match(result.suggestion, /重新写入 zlk-\*/);
  } finally {
    server.close();
  }
});

...
async function localCandidates(root: string): Promise<Candidate[]> {
  ...
  for (const baseRelative of ["simple_cluster/tmp", "tmp"]) {
    ...
    let baseStat;
    try { baseStat = await fs.lstat(base); } catch { continue; }
...
async function deleteLocalCandidate(root: string, row: Candidate): Promise<void> {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || token !== row.token || stat.mtimeMs > Date.now() - 7 * 86400000) throw new Error(`本机候选已变化：${row.path}`);
  const parent = path.dirname(full);
  const leaf = path.basename(full);
  if (!leaf || leaf === "." || leaf === ".." || leaf.includes("/") || leaf.includes("\\") || await fs.realpath(parent) !== parent || !parent.startsWith(realRoot + path.sep)) throw new Error("PARENT_CD_FAILED");
...
export function openCacheCleanupPanel(client: any, endpointProvider: () => Endpoint[], localRoot?: string): void {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const errors: string[] = [];
      if (localRoot) {
        try { rows.push(...await localCandidates(localRoot)); }
        catch (error) { errors.push(`本机: ${String(error)}`); }
      }
      settled.forEach((result, index) => {
        if (result.status === "rejected") errors.push(`${endpoints[index].id}: ${String(result.reason)}`);
        else for (const row of result.value.rows || []) rows.push({ ...row, workerId: result.value.id });
      });

...
export function previewResultParse(text: string, sourceFile: string, preset: ResultParserPreset): ResultParsePreview {
...
export function buildResultDashboard(records: ExperimentResultRecord[], issues: ResultValidationIssue[] = [], schema?: ResultSchema): ResultDashboardSummary {
  const bySuite = new Map<string, ExperimentResultRecord[]>();
  for (const record of records) bySuite.set(record.suite, [...(bySuite.get(record.suite) || []), record]);
...
function parseJsonResult(text: string, sourceFile: ExperimentResultRecord["sourceFiles"][number], preset: ResultParserPreset, parserConfig: ResultParserConfig): ExperimentResultRecord[] {
  ...
  return rows.map((row: any) => {
    ...
    for (const [metric, raw] of Object.entries(row.metrics || row)) {
      ...
      if (typeof value === "number") record.metrics[alias(metric, preset)] = metricValue(value, alias(metric, preset), row, m, sourceFile.path, preset);
    }
    return record;
  });
}
...
function idsFromRow(row: Record<string, any>, m: ResultParserPreset["columnMapping"], sourcePath: string) {
...
function metricValue(value: ResultMetricValue["value"], metric: string, row: Record<string, string>, m: ResultParserPreset["columnMapping"], sourceFile: string, preset: ResultParserPreset): ResultMetricValue {
  return {
    value,
...
function standardDimensions(row: Record<string, any>, m: ResultParserPreset["columnMapping"]): Record<string, string | number | boolean> {
  ...
  for (const key of ["suite", "dataset", "split", "fold", "seed"] as const) {
...
function alias(metric: string, preset: ResultParserPreset): string {
  ...
  if (ci) return ci[1];
  if (lower === "dsc" || lower === "dice") return "DSC";
  if (lower === "hd95") return "HD95";
...
function directionFor(metric: string, preset: ResultParserPreset): "higher" | "lower" {
  return preset.metricDirections?.[metric] || (classificationMetricDirections[metric] || segmentationMetricDirections[metric] || "higher");
}

...
export type ScopeStatus = { state: "same" | "different" | "remote-only" | "unknown"; detail: string; versions?: Record<string, { sha256: string; modifiedAtMs: number; latest?: "plan" | "local" | "candidate" | "same" | "manual" }>; held?: boolean; unverified?: boolean };
...
function paintStatus(badge,health,directory){if(!health){badge.textContent='待刷新';return}if(health.state==='same')return;if(directory){const parts=health.detail.split(' · '),facts=[];for(const part of parts){const count=Number(part.match(/^([0-9]+) /)?.[1]||0);if(!count)continue;if(part.includes('待更新或冲突'))facts.push(count+' 个文件内容不同或缺失');else if(part.includes('仅 Worker 一致'))facts.push(count+' 个文件本机缺失或不同');else if(part.includes('未确认'))facts.push(count+' 个文件未完成校验');else if(part.includes('范围外'))facts.push(count+' 个文件未纳入同步范围')}const outside=health.detail.match(/^当前同步范围外 · ([0-9]+) 个文件$/);badge.textContent=facts.join(' · ')||(outside?outside[1]+' 个文件未纳入同步范围':health.detail);return}const parts=health.detail.split(' · ').filter(part=>!part.endsWith(' 最新版')&&!part.endsWith(' 同版')&&part!=='Worker 内容基准');for(let part of parts){const missing=part.match(/^(.+) 待更新$/);if(missing)part=missing[1]+(health.versions?.[missing[1]]?' 内容不同':' 缺失');else if(part==='内容冲突，无法判定最新版')part='最新版未确定';else if(part==='本机 不同版')part='本机内容不同';const item=document.createElement('span');item.textContent=part;badge.appendChild(item)}}
...
function row(entry,depth){
  ...
  if(entry.path!=='.'&&view.detailPath===entry.path){
    ...
    for(const id of locations){const v=health?.versions?.[id],item=document.createElement('span');item.className='version '+(v?.latest==='candidate'?'candidate':v?.latest?'same':'');item.textContent=id+(v?' · '+(v.modifiedAtMs?new Date(v.modifiedAtMs).toLocaleString():'时间未知'):entry.directory&&health&&health.state!=='unknown'?' · 目录内容已校验':' · 尚未校验');
...
window.addEventListener('message',event=>{
  ...
  if(message.type==='init'){
    roots=message.roots;
    for(const root of roots)views.set(root.id,{selected:new Set(root.selected),excluded:new Set(),expanded:new Set(['.']),children:new Map(),listPending:new Set(),pending:new Set(),refreshQueue:[],mutationParent:null,focus:null,busyAction:null,statuses:{}});

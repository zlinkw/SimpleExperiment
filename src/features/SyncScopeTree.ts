...
export type ScopeStatus = { state: "same" | "different" | "remote-only" | "unknown"; detail: string; versions?: Record<string, { sha256: string; modifiedAtMs: number; latest?: "plan" | "local" | "candidate" | "same" | "manual" }>; held?: boolean };
export type ScopeRoot = {
  ...
  remove?: (relative: string, endpointId: string, directory: boolean) => Promise<void>;
  retain?: (relative: string, endpointId: string, directory: boolean) => Promise<void>;
};

export function openSyncScopeTree(title: string, roots: ScopeRoot[], refreshIntervalMs: number): void {
  ...
  panel.webview.onDidReceiveMessage(async (message: any) => {
    ...
    try {
      ...
      if (message.type === "list") {
        ...
        if (!endpointId) throw new Error("请明确选择一个目标位置。");

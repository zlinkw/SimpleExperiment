...
export interface PlottingOutputContract {
  ...
  files: Record<string, { path: string; description: string; fields: string[] }>;
  requiredFields: readonly string[];
  notes: string[];
}

export function buildPlottingOutputContract(generatedAt = new Date().toISOString()): PlottingOutputContract {
  return {
    schemaVersion: 1,
    generatedAt,
    consumer: "D:/GitRepo/my_ppt_app",

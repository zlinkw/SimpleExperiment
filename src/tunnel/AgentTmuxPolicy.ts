...
export function defaultAgentTmuxSessionName(role: AgentTmuxRole, endpointId?: string): string {
  if (role === "hub") return "zlk-hub-agent";
  return `zlk-worker-${slug(endpointId || "worker")}-agent`;
}

export function isValidZlkTmuxSessionName(value: string): boolean {
...
function agentRuntimeCommand(options: {
  ...
  return [
    ...
    `AGENT_SCRIPT="$INSTALL_DIR/${zlkAgentRuntimeRelativePath}"`,
...
type ZlkCondaHookShell = "posix" | "bash";

interface ZlkCondaActivationOptions {
  quietActivate?: boolean;
  reportOptionalFailure?: boolean;
  reportOptionalMissing?: boolean;
  hookShell?: ZlkCondaHookShell;
}

function foregroundBashShellCommand(condaEnv = zlkDefaultCondaEnv): string {
...
function zlkCondaActivationShell(required: boolean, options: ZlkCondaActivationOptions = {}): string {

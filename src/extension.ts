...
import ExtensionUpdates_1 = require("./features/ExtensionUpdates");
import RunOperations_1 = require("./features/RunOperations");
...
} as Record<string, { store: "global" | "workspace"; label: string; writable: boolean }>;
...
class RealtimeTunnelPanelProvider {
    ...
    private taskSelectionRevision = 0;
    private taskSelectionDerivedCache;
    ...
    private planFileWatcherGeneration = 0;
    planLocalChangeParseTimer;
    private planLocalChangeParseGeneration = 0;
    recentPlans = [];
    resultsSummary;
    ...
    private localOperationsRevision = 0;
    private planRuntimeEvidenceCache;
    ...
    private postLaunchAutoTestGeneration = 0;
        ...
        return {
            ...
            "operations.list": async (params = {}) => {
                ...
                let filtered = rows.filter((row) => (
                    (!filters.operationId || row.operationId === filters.operationId || String(row.remoteOperationId || "") === filters.operationId)
    ...
    apiStateValue(name) {
        ...
        const stateKey = (keys as Record<string, string>)[name];
        if (!meta || !stateKey)
    ...
    async apiStateSet(params = {}) {
        ...
        const stateKey = (keys as Record<string, string>)[key];
    ...
    apiPrepareServerTargets(topology, serverIds, workerTunnels, setup = this.setupConfig) {
        ...
        this.assertExecutionCondaEnvReady(
            this.apiMergedWorkerConfigs(params.workerTunnels, setup)
    ...
    async apiPrepareConfirmationPreview(params = {}) {
        try {
            ...
            const apiError = error as { apiCode?: unknown; apiData?: unknown };
    ...
    async apiProjectBootstrap(params = {}) {
        ...
        if (params.confirm !== true)
            throw confirmationRequired({
                ...
                requires: ["confirm"],
                missing: [
                    ...((preparePreview as { missing?: unknown[] })?.missing || []),
                    ...validation.missing,
                ],
    ...
    private cancelPostLaunchAutoTest() {
    ...
    async checkPluginUpdates(manual = false) {
        ...
        try {
            ...
            const experiment = ExtensionUpdates_1.componentUpdate(
                ExtensionUpdates_1.EXPERIMENT_EXTENSION_ID,
                ExtensionUpdates_1.EXPERIMENT_UPDATE_REPO,
                "SimpleExperiment",
                currentVersion(ExtensionUpdates_1.EXPERIMENT_EXTENSION_ID),
                experimentRelease,
                "simple-experiment",
            );
            const sftp = ExtensionUpdates_1.componentUpdate(
                ExtensionUpdates_1.SFTP_EXTENSION_ID,
                ExtensionUpdates_1.SFTP_UPDATE_REPO,
                "SimpleSFTP",
                currentVersion(ExtensionUpdates_1.SFTP_EXTENSION_ID),
                sftpRelease,
                "simple-sftp",
            );
    ...
    runOperationWorkerId(record) {
        ...
        if (explicit) return this.resolveWorkerEndpointId(explicit) || explicit;
    ...
    private xshellLibraryRequestKey(dirs = xshellScanDirs(this.setupConfig), configuredPaths = this.configuredXshellSessionPaths()) {
    ...
    private buildPlanRuntimeEvidenceState() {

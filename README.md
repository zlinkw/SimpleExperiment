...
结果区的“全项目最终结果 · final”是主入口。插件通过每个 Plan 的可信 case、seed、方法和端点，跨已启用 Worker 去重后重新计算均值与样本标准差；Plan 的原始表不会被改写。全项目 CSV 与 Markdown 分别位于 experiments/results/final/final.csv、final.md。每个方法也有 experiments/results/<方法>/<方法>.csv、.md。行按方法、数据集、训练比例和评估端点显示，重名时增加 case；缺 seed 时 jobs 显示实际/计划数。同一 seed 的指标若在不同 Worker 或 Plan 间冲突，会阻止覆盖总表并提示核查。当前 Plan 自动刷新结果时会更新本机表，亦可点击“重建所有方法与 final”逐个查询所有 Plan。
...
结果区的打开按钮会先确认远端来源与本机目标，再经现有 Agent 隧道下载。当前 Plan 的原始逐 seed 表进入 experiments/results/<方法>/raw/，详细聚合进入 detail/，单 Plan 计算副本进入 trace/。多 Worker 的同名文件再按 Worker 分目录。点击“同步当前 Plan 全部结果到本机”一次同步这些文件，已有文件统一询问覆盖或只补缺失。远端训练程序原有输出目录不变；旧版已存在的平铺文件保留，不自动删除。

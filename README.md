...
运行环境准备区的“补充上传代码”用于运行 Plan 时缺少模块或配置的情况：从当前工作区添加文件或目录，文件和目录均可一次多选；也可查看已添加路径和实际纳入文件、移除路径。普通 YAML/JSON 配置无需在文件名中包含 config。用户可在同一入口配置允许的扩展名和单文件大小上限；默认允许常见源码、配置、脚本与文本格式，默认上限 2 MB，扩展名填写 `*` 可允许任意类型。路径必须位于当前项目内；符号链接、`.env`、`.git`、`.vscode` 与插件状态目录保持硬保护。设置保存在插件工作区配置中，不要求项目提供专用 YAML。默认代码扫描继续生效。同步前检查远端文件冲突，同步后核对上传源码哈希。

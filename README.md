...
当前 Plan 的结果区有独立的“当前 Plan 简洁汇总”主卡片，两个主按钮分别为“查看简洁汇总 CSV”和“查看简洁汇总 Markdown”。表格按方法、数据集、训练比例和评估端点逐行展示均值与样本标准差；需要区分同名行时自动补入 case 等维度。原始 seed 表保持不变，`seed_mean_std.csv` 详细汇总在“原始数据与详细追溯”折叠区。缺少 seed 时仍输出均值与标准差，并把 `jobs` 标记为 `n/计划 seed 数`；各指标的参与数以详细表为准。“跨 Plan 项目汇总”另有独立折叠区，多 Worker 时每台 Worker 各有一份，不代表跨服务器合并结果。Plan 完成后自动生成，也可点“重建当前 Plan 汇总”。
...
结果区的“打开”按钮先确认远端来源与本机目标，再同步到本地项目的 `experiments/results/` 并打开。原始表保留原名；Plan 简洁结果命名为 `<Plan 名>_final.csv` 和 `<Plan 名>_final.md`，项目简洁总表为 `project_final.csv` 和 `project_final.md`；详细表仍为 `<Plan 名>_seed_mean_std.csv` 与 `project_seed_mean_std.csv`。多 Worker 时放在各自的 Worker 子目录。已有本地目标时会明确提示覆盖，也可只打开现有文件。旧的 `experiments/results.csv` 属于兼容候选文件，插件不会自动删除或迁移已有文件。

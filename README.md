...
当前 Plan 的结果区以“简洁 CSV”和“可读 Markdown”为主入口，按方法、数据集、训练比例和评估端点逐行展示均值与样本标准差；需要区分同名行时自动补入 case 等维度。原始 seed 表保持不变，`seed_mean_std.csv` 详细汇总继续用于追溯。缺少 seed 时仍输出均值与标准差，并把 `jobs` 标记为 `n/计划 seed 数`；各指标的参与数以详细表为准。项目简洁总表汇集当前 Worker 已生成的各 Plan；多 Worker 时每台 Worker 各有一份，不代表跨服务器合并结果。Plan 完成后自动生成，也可点“重建汇总”。

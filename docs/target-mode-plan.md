...
## 当前批次：project-148（已完成）
### 修复点

- 将 Plan 校验预演与正式提交命令拆分为后端固定集合。
- 由两个子集合派生完整调度集合，并复用命令超时、运行前门禁和提交后导航。
- 增加集合组成和后端命令路由回归。
...
- 校验与预演仍使用 90 秒本地命令看门狗，运行与复现仍使用 150 秒。
- 只有运行与复现执行正式提交门禁并在提交后定位任务区。
...
- [已通过] TypeScript 构建。
- [已通过] 后端 Plan 命令集合与路由定向 Node 测试，4/4。
- [已通过] `git diff --check`。
...
- 本批仅处理后端 Plan 命令固定集合、对应测试和计划文档；无视觉变化，不调用截图。
- project-148 构建与 4/4 定向测试通过；下一批仅处理后端即时结果刷新命令集合。

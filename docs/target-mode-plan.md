...
- project-231 在单次与批量运行前逐 Plan 选择在线 Worker；只同步目标 Worker，并由目标 Worker 独立校验、预演和调度完整 Plan。

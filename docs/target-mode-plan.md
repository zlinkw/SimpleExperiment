...
## 当前批次：autonomous-static-022（已完成）

### 范围
- Plan 选择和任务选择项目态分别使用单飞写入队列。
- 写入期间的新选择合并到后续一轮，避免旧快照晚完成后覆盖新选择。
- 项目切换后旧项目队列不能消费新项目状态，文件格式和路径保持不变。
...
### 相邻回归风险
- 调用方 `await` 必须等待同项目内合并后的最后一轮写入。
- 旧项目失败不得污染新项目 dirty 状态；当前项目失败仍允许后续调用重试。

### 验证清单
- [已通过] TypeScript、Lint、生成 JavaScript 语法及项目隔离定向测试 13/13。
- [跳过] 广泛测试；本周期第 5 批执行。

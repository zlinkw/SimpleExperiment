...
## 当前批次：autonomous-static-017（已完成）

### 范围
- 任务监控的“当前版本/全部任务”范围在 Webview 重建后恢复。
- 结果轨迹的“当前 Plan/全部记录”范围使用同一持久化机制。
- 提交任务、树导航和 Extension Host 导航引发的范围变化同步更新本地 Webview 状态。
...
### 相邻回归风险
- 非法或旧版持久化值必须回退到 `selected`，不能扩大默认数据范围。
- 视图偏好只保存在 VS Code Webview，不进入项目文件、Extension Host 或服务器请求。

### 验证清单
- [已通过] TypeScript、Lint、内联脚本语法及 Plan 范围定向测试 13/13。
- [跳过] 广泛测试；本周期第 5 批执行。

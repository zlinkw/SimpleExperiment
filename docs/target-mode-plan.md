...
- Xshell 会话库扫描固定目录、显式会话路径和项目世代，扫描结果、错误与 postState 仅写回当前上下文。
- 新请求键不得复用旧键的 in-flight 扫描，旧 Promise 完成不得清空新扫描引用。
- 保持历史 VSIX、`zlk_cluster/ui/` 和真实服务器不变。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 本批只使用静态测试验证本机会话文件扫描隔离，不启动 Xshell 或服务器连接。
...
- [已通过] Xshell 会话库请求键、in-flight 和项目世代静态契约测试，17/17。
- [已通过] TypeScript、Lint、Node 语法与 `git diff --check`。

## 本批记录
- 本轮只处理 Xshell 会话库扫描上下文，不改变会话解析与配置入口。
- 真实服务器行为保持 `needs field verification`。
- 下一批边界：前端 GPU 曲线样式持久化写入合并。

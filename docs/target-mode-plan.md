...
## 当前批次：autonomous-static-037（已完成）

### 范围
- 让任务区签名计算和实际渲染复用同一范围、选择与优先级视图模型。
- 将任务范围纳入本地渲染签名，确保范围切换触发正确刷新。
- 保持任务状态计数、选择优先级、旧任务隐藏与详情定位语义不变。

...
## 当前批次：project-003（已完成）
### 修复点

- 离线 bundle 文件选择、解析、暂停实时流与项目状态写回固定项目世代和工作区根。
- 旧工作区导入结果不得暂停、覆盖或刷新新项目的离线状态。
- 保持历史 VSIX、`zlk_cluster/ui/` 和真实服务器不变。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 本批只验证离线 bundle 本地导入与项目状态写回静态契约，不访问网络。
...
- [已通过] 离线 bundle 导入项目隔离测试，11/11。
- [已通过] TypeScript、Lint、Node 语法与 `git diff --check`。

## 本批记录
- 本轮只处理离线 bundle 导入生命周期，不改变 bundle 格式和离线模式语义。
- 真实服务器行为保持 `needs field verification`。
- 下一批边界：前端配置筛选 debounce 的过期重绘抑制。

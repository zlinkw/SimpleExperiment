...
## 当前批次：resilience-003（已完成）
### 修复点

- 结果页签名只保留当前 Plan、结果相关选择、结果摘要、追踪和绘图状态。
- 去除每分钟强制失效、无消费者的 `selectedTraceKey`、全量 Plan 和全量 capability 签名。
- 任务专用选择字段变化不得触发结果页重绘。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 当前 Plan 版本、结果行、归档状态、追踪选择和 PPT 状态变化仍必须刷新结果页。
- 实验记录相对时间随真实结果或追踪状态更新，不单独制造定时重绘。
- archived-only 数据源和结果操作能力判断保持不变。
- 当前仅执行静态验证，不连接服务器或重载、关闭 VS Code。

### 验证清单

- [已通过] 结果签名依赖、摘要工作台缓存和相邻结果工作流定向测试，37/37。
- [已通过] TypeScript 构建、Lint、Node 语法与 `git diff --check`。

## 本批记录
- 本轮只压缩结果页渲染签名，每五批执行一次完整测试。
- 真实服务器行为保持 `needs field verification`。
- 下一批边界：收紧拓扑切换后的本地缓存失效边界，不扩展文件传输或服务器连接范围。

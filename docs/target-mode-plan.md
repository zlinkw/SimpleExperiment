...
## 后续优先级
- [已完成] 1/5 后端：复用端口分配与冲突派生。
- [待做] 2/5 前端：审计轻量状态派生缓存。
- [待做] 3/5 后端：审计运行态读取边界。
- [待做] 4/5 前端：审计渲染查找与索引。
- [待做] 5/5 前后端：执行第十八轮非服务器静态测试。

## 当前批次：autonomous-static-085（已完成）
### 修复点

- 按标准化服务器配置引用缓存当前端口分配派生。
- 按分配引用和端口范围缓存静态端口冲突，避免状态构建重复计算。
- 增加复用、配置替换和端口范围变化失效测试。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 禁用 Worker 不得进入当前分配结果。
- 配置或端口范围变化后不得保留旧分配与冲突。
- 当前仅执行静态验证，不连接服务器或重载、关闭 VS Code。

### 验证清单

- [已通过] TypeScript 构建和生成文件一致性。
- [已通过] 端口分配、冲突、端点注册、状态发布和服务器视图定向测试 15/15。
- [已通过] Lint 与 `git diff --check`。

## 本批记录
- 同一标准化配置只派生一次启用端点分配；状态构建中的 assignments 与 conflicts 共享结果。
- 配置引用替换后重建分配；端口范围原地变化也会单独使冲突缓存失效。

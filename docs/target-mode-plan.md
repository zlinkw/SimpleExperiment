...
## 当前批次：project-147（已完成）
### 修复点

- 将同步未开始、执行中和未参与状态提升为前端固定集合。
- 复用发布流程、概览和 Plan 运行区共用的同步状态判定。
- 增加同步状态集合组成与调用回归。
...
- 待同步、执行中、未知、跳过和未参与状态仍不得判定为同步成功。
- 包含 `fail`、`error`、失败、错误、未参与或 `skip` 的状态仍不得判定为成功。
...
- [已通过] TypeScript 构建。
- [已通过] 前端同步状态固定集合、缓存与内联脚本定向 Node 测试，15/15。
- [已通过] `git diff --check`。
...
- 本批仅处理前端同步未就绪状态固定集合、对应测试和计划文档；无视觉变化，不调用截图。
- project-147 构建与 15/15 定向测试通过；下一批仅处理后端 Plan 命令集合。

...
## 当前批次：project-219（已完成）
### 修复点

- 审计前后端派生热点，收敛每次动作 payload 内重复读取七次 scheduler 配置的问题。
- 单个动作只读取一次调度配置快照，再透传全部轮询、TTL、事件延迟和 Worker 节流字段。
- 保持动作 payload 字段、数值约束、Worker 目标和结果目录行为不变。
- 保持未跟踪历史安装包、VSIX、`zlk_cluster/ui/` 和真实服务器不变。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 单次快照必须覆盖原有七个 scheduler 字段，不得漏传或改变键名。

...
## 当前批次：autonomous-static-023（已完成）

### 范围
- 任务工作台单次扫描生成状态统计、选中项、关键项、排队项和活动项。
- 复用同一视图模型生成渲染签名、任务列表和详情候选，保持现有优先级与数量预算。
...
### 相邻回归风险
- 选中任务、运行或失败任务、排队任务的显示优先级和 80 条预算必须保持不变。
- 状态计数、详情默认项和任务签名不得改变语义。

### 验证清单
- [已通过] TypeScript、Lint、生成 JavaScript 语法、内联脚本 2/2 及任务定向测试 10/10。
- [跳过] 广泛测试；本周期第 5 批执行。

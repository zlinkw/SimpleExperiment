...
## 当前批次：history-006
### 修复点
- 增加可复现的 GPU 历史曲线浏览器预览，覆盖总体曲线、单卡曲线、四台服务器和离线缺口。
- 在桌面、高 DPI 和窄侧栏宽度检查图例、文本、曲线及相邻区域是否重叠。
- 预览只使用模拟运行状态，不写入结果 CSV、归档、论文证据或插件安装目录。

### 回归风险
- 相邻回归风险：视觉夹具必须隔离 `acquireVsCodeApi` 和真实命令副作用。
- 模拟数据不得被误记为真实服务器连续三天采样证据。
- 窄侧栏不能依赖横向滚动隐藏图例、摘要或 GPU 指标。

...
## 后续优先级
- [已完成] 1/5 后端：审计 Agent 写入路径的缓存失效。
- [待做] 2/5 前端：审计任务与结果行渲染预算。
- [待做] 3/5 后端：审计 Scheduler 运行时派生。
- [待做] 4/5 前端：审计操作与事件视图派生。
- [待做] 5/5 前后端：执行第十九轮非服务器静态测试。

## 当前批次：autonomous-static-090（已完成）
### 修复点

- 新增 `invalidate_runtime_json_cache`，写入后按绝对路径丢弃只读 JSON 缓存条目。
- `atomic_write` 与 `move_file_replace` 在替换目标文件后主动失效对应缓存。
- 覆盖粗粒度文件时间戳下 inode、大小、mtime 均未变化的陈旧读取风险。
- 增加连续写入、上传落地覆盖与写入辅助函数接线测试。

### 相邻回归风险

- 写入后首次读取必须返回新内容，不得沿用同签名旧值。
- 失效逻辑不得影响未缓存路径或抛出异常中断写入。
- 当前仅执行静态验证，不连接服务器或重载、关闭 VS Code。

### 验证清单

- [已通过] TypeScript 构建、Agent 运行时生成与校验和一致性。
- [已通过] Agent 运行时与文件传输定向测试 38/38。
- [已通过] Lint 与 `git diff --check`。

## 本批记录
- stat 签名失效在 1 秒粒度文件系统上不足以覆盖同尺寸写入，改为写入侧主动失效。
- 上传落地路径复用同一失效入口，避免覆盖后仍返回旧摘要。

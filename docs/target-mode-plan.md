...
## 当前批次：panel-onboarding-handshake-004
### 修复点
- [已完成自动化] Webview 先注册消息监听，再赋值 HTML；握手同步到达时不得丢失，已加入顺序回归测试。
- [已完成自动化] 项目接入通知改为模态窗口；取消或关闭通知时，面板保留“当前项目待接入”持久入口。
- [已保护] 仅修改面板启动和本地提示状态；服务器连接、Xshell、SimpleSFTP 上传、GPU、归档、结果和 PPT 行为未执行现场联调。
...
- 本批目标与限制：先锁定握手竞态，再恢复可靠的项目接入可见性；面板和当前项目提示仅做静态验证，真实通信保持 `needs field verification`；代码批提交号待回填，包版本留待后续发布批次。

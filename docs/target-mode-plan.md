...
- 结果摘要延迟刷新绑定定时器世代、项目世代和发起客户端，旧回调不得作用于新客户端。
- 客户端重置时清理旧定时器与旧请求占用标记，并为仍待处理的摘要事件恢复刷新。
- 保持历史 VSIX、`zlk_cluster/ui/` 和真实服务器不变。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 客户端重置可能发生在摘要请求或重试计时尚未完成时，旧回调不得阻塞新客户端刷新。
...
- [已通过] 结果摘要 timer、in-flight 与客户端重置静态契约测试，8/8。
- [已通过] TypeScript、Lint、Node 语法与 `git diff --check`。

## 本批记录
- 本轮只处理结果摘要刷新 timer、in-flight 和客户端重置权威。
- 真实服务器行为保持 `needs field verification`。
- 下一批边界：计划文件 watcher 与 debounce 在工作区切换后的旧回调审计。

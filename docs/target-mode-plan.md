...
## 当前批次：project-014（已完成）
### 修复点

- 输出接入模板生成固定项目上下文，逐段校验模板加载、写入确认、逐文件写入和元数据刷新。
- 旧项目模板任务不得向新项目发通知、排队结果解析或打开同名 README。
- 保持历史 VSIX、`zlk_cluster/ui/` 和真实服务器不变。
- 不生成或安装 VSIX，不连接服务器，不重载或关闭 VS Code。

### 相邻回归风险

- 本批只验证本地模板生成静态上下文契约，不连接服务器。
...
- [已通过] 输出接入模板生成上下文隔离测试，22/22。
- [已通过] TypeScript、Lint、Node 语法与 `git diff --check`。

## 本批记录
- 本轮只处理输出接入模板生成生命周期，不改变模板内容、文件集合和备份规则。
- 真实服务器行为保持 `needs field verification`。
- 下一批边界：第三十三轮完整非服务器静态测试。

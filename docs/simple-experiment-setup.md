...
每个 VS Code 窗口只打开一个实验项目。多根工作区会在快速接入、Agent 路径写入、SFTP 上传和远端实验操作前被阻断；请把目标实验项目单独打开，避免上传目录、Plan 与项目状态指向不同文件夹。

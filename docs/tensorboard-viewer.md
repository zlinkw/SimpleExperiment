...
图表底部悬停详情使用紧凑对比表，每个 case 占一行，列出 step、数值、平滑值、样本标准差、参与 seed 数及各 seed 原值。case 和 step 列按当前内容收紧，其余宽度分给数值列。每个 seed 有固定的独立列，蓝色加粗表头标明 seed 编号；同一步缺少该 seed 时显示“—”，因此不同 case 的同一 seed 始终纵向对齐。窄卡片保留 case、step、数值和参与 seed 数，平滑、标准差及各 seed 原值列会按宽度隐藏。表格不设内部纵向滚动。悬停表格行可查看完整字段和精度，或把该图切换成单行显示查看更多列。

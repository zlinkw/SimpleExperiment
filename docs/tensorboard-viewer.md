...
页面按 Plan → case → 指标组织。首次打开只查询目录；点击 case 将它设为主图，自动列出并打开该 case 的**全部指标图**。其他 case 的“加入对比”按钮只叠加同名指标的均值曲线，最多可同时加入 7 个；按钮可再次点击移除，也可清空全部对比。左侧搜索框可按 Plan 或 case 名过滤，“仅看已选”只显示主图和已加入对比的 case。拖动侧栏右边界可调整宽度，也可用方向键调整；宽度在本机浏览器保存。长名称会分行，seed 数单独显示。

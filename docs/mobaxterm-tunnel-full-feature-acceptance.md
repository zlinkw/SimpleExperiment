...
1. 配置 MobaXterm
   - 操作：在插件中运行“配置 MobaXterm 实时隧道”，优先选择本机已保存的 SSH 服务器；没有合适条目时填写 `MobaXterm.exe`、服务器 IP/域名、登录用户名、SSH 端口号、本地端口、服务器上的 Agent 端口。
   - 预期结果：命令预览显示 `127.0.0.1:<localForwardPort>:127.0.0.1:<remoteAgentPort>`。
   - 相关 API：无。
   - 失败检查：确认 exe 路径、端口范围、服务器 IP/域名、登录用户名、本机 SSH 配置名称。

2. 启动隧道并检测
   - 操作：点击“启动 MobaXterm”，确认命令预览后运行“检测隧道”。
   - 预期结果：本地端口打开，`/api/health` 正常，`/api/capabilities` 正常。
   - 相关 API：`GET /api/health`、`GET /api/capabilities`。
   - 失败检查：确认 MobaXterm 窗口是否显示认证错误，Hub Agent 是否启动，token 是否正确。
...
10. 离线导入
    - 操作：关闭 MobaXterm，导入 offline bundle。
    - 预期结果：插件不访问网络，离线结果仍可查看。
    - 相关 API：无。
    - 失败检查：确认 bundle JSON 格式。

11. 关闭 MobaXterm 后确认无回退
    - 操作：关闭隧道后运行检测和手动刷新。
    - 预期结果：只提示修复 MobaXterm 隧道或使用离线导入，不出现 direct SSH fallback。

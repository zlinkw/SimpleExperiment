...
本文件只记录候选，不授权 Codex 删除、移动、清空或暂存删除任何项目文件。用户应逐项审核，并仅由用户本人手动删除。后续发现确认无用的文件时，继续追加准确的仓库相对路径，禁止使用通配符代替路径。
...
1. 只记录准确的仓库相对路径，不记录通配符或整个仓库根目录。
2. 每项必须写明原因、风险、依赖、发现日期和状态。
3. Codex 不执行删除、不移动到回收站、不清空内容、不使用 `git rm`，也不暂存删除状态。
4. 用户手动删除后，如需提交删除记录，应另行明确要求，并先检查 Git diff 与依赖。

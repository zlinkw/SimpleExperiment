...
每个 job 的输出使用 `原输出目录/attempts/<运行 ID>/`，重跑保留旧 attempt。项目适配器须在该目录写 `best_model.pth`、`test_results/formal_result_rows.csv`、`test_results/four_state_metrics.csv`，并保留原始 TensorBoard 事件与日志。正式指标行和四态行的列、唯一键、双测试端点及检查点关系由项目校验。插件用 SimpleSFTP 读取 SHA256 清单并同步当前 attempt 的文件，逐目标校验内容；离线 Worker 保留待镜像状态。同步使用文件清单，不执行目录删除。
